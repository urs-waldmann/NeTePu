...
CAMS = [1, 3, 6, 10, 12, 13, 15, 19, 20, 22]
nkps = CFG['data']['nkps']
...
mode = 'track_texture_novel_views'
...
# n_samples = 132
# print('Show {} samples from "track texture novel views" set.'.format(n_samples))
# view_dataset(
...
with torch.no_grad():
    for i, data in enumerate(loader):
        ...
        for c_idx, cam_name in enumerate(CAMS):
            ...
            z_texture_all[int((frame - 1) / 90), texture.item(), c_idx, :] = z_texture.squeeze().detach().cpu().numpy()

    np.save(
        os.path.join(
            exp_dir,
            'z_texture_novel_views.npy'
        ),
        z_texture_all
    )
