...
Implementation extended to texture disentanglement
Author: Urs Waldmann (urs.waldmann@uni.kn)
'''
...
class CrossTransformerBlock(nn.Module):
    ...
    def forward(self, xyz_q, lat_rep, xyz, points):
        ...
        # pos_encode = self.fc_delta(
        #     torch.cat([xyz_q[:, :, None] - xyz[:, :, :, :2], xyz[:, :, :, -1:]], dim=-1)
        # )  # b x nQ x k x dim
...
class NePuRenderer(nn.Module):
    ...
    def __init__(self, lat_dim, loc_lat_dim, dim_space, nneigh=7, hidden_dim=64, n_blocks=5):
        ...
        self.pos_enc_head = RenderHead(loc_lat_dim=loc_lat_dim, out_dim=3, hidden_dim=hidden_dim, n_blocks=n_blocks)
    ...
    def forward(self, xyz_q, encoding, camera_params, kps_2d):
        ...
        keyPos_hom = torch.cat(
            [xyz, torch.ones([xyz.shape[0], xyz.shape[1], 1], device=xyz.device, dtype=torch.float)],
            dim=2
        ).permute(0, 2, 1)
        ...
        pos_enc = self.pos_enc_head(lat_rep)

        return occ, depth, pos_enc


class NePuTextureRenderer(nn.Module):
    def __init__(self, lat_dim, loc_lat_dim, dim_space, nneigh=7, hidden_dim=64, n_blocks=5):
        super().__init__()
        self.dim_space = dim_space
        self.n_blocks = n_blocks

        self.down_texture = nn.Linear(lat_dim, loc_lat_dim)

        self.ct1_texture = CrossTransformerBlock(loc_lat_dim, loc_lat_dim, None, nneigh=nneigh)

        self.color_head = RenderHead(loc_lat_dim=loc_lat_dim, out_dim=3, hidden_dim=hidden_dim, n_blocks=n_blocks)

        self.trans2d_1_texture = TransformerBlock(loc_lat_dim, 8, 3)
        self.trans2d_2_texture = TransformerBlock(loc_lat_dim, -1, 3, group_all=True)
        self.trans2d_3_texture = TransformerBlock(loc_lat_dim, -1, 3, group_all=True)

    def forward(self, xyz_q, encoding, encoding_texture, camera_params, kps_2d):
        xyz = encoding['anchors']

        # get global feature vectors for texture
        lat_rep_texture = self.down_texture(encoding_texture['z_texture'])
        # get local feature vectors for texture
        feats_texture = encoding_texture['anchor_feats_texture']
        ...
        feats_texture = self.trans2d_1_texture(kps_2dd, feats_texture)
        feats_texture = self.trans2d_2_texture(kps_2dd, feats_texture)
        feats_texture = self.trans2d_3_texture(kps_2dd, feats_texture)

        # CVA with global latent code for texture
        lat_rep_texture = self.ct1_texture(xyz_q, lat_rep_texture, kps_2dd, feats_texture)

        color = self.color_head(lat_rep_texture)
...
class NePuTextureDecoder(nn.Module):
    def __init__(self, d_model, nkps, loc_lat_dim):
        super().__init__()

        self.mlp_feats_texture = nn.Sequential(
            nn.Linear(d_model, 2 * d_model),
            nn.ReLU(),
            nn.Linear(2 * d_model, 4 * d_model),
            nn.ReLU(),
        ...
        self.t1_texture = TransformerBlock(loc_lat_dim, 8, 3, group_all=False)
        # self.t2_texture = TransformerBlock(200, 8, 3, group_all=False)
        self.t3_texture = TransformerBlock(loc_lat_dim, -1, 3, group_all=True)
        self.t4_texture = TransformerBlock(loc_lat_dim, -1, 3, group_all=True)

        self.nkps = nkps
        self.loc_lat_dim = loc_lat_dim

    def forward(self, z_texture, encoding):
        pos_init = encoding['anchors']
        feats_texture = self.mlp_feats_texture(z_texture).view(-1, self.nkps, self.loc_lat_dim)
        pos = pos_init * 4
        feats_texture = self.t1_texture(pos, feats_texture)
        # feats_texture = self.t2_texture(pos, feats_texture)
        feats_texture = self.t3_texture(pos, feats_texture)
        feats_texture = self.t4_texture(pos, feats_texture)

        return {'z_texture': z_texture, 'anchor_feats_texture': feats_texture}


def get_encoder(CFG):
    CFG_enc = CFG['encoder']
    encoder = NePuEncoder(nfinal_transformers=CFG_enc['nfinal_trans'],
                          d_transformer=CFG_enc['loc_lat_dim'],
                          d_space=3,
...
def get_decoder(CFG):
    ...
    d_kps_bottle = CFG_dec.get('kps_bottle', int(dim/2))
    loc_lat_dim = CFG_enc.get('loc_lat_dim', 200)
    if CFG['decoder']['type'] == 'lfn':
...
def get_texture_decoder(CFG):
    CFG_enc = CFG['encoder']
    CFG_dec = CFG['decoder']
    dim = CFG_enc.get('lat_dim', 512)
    loc_lat_dim = CFG_dec.get('loc_lat_dim', 200)
    if CFG['renderer']['type'] == 'lfn':
        decoder = DecoderDummy()
    else:
        decoder = NePuTextureDecoder(
            d_model=dim,
            nkps=CFG['data']['nkps'],
            loc_lat_dim=loc_lat_dim
        )
...
def get_renderer(CFG):
    ...
    if CFG_render['type'] == 'nepu':
        decoder = NePuRenderer(
            lat_dim=CFG_enc['lat_dim'],
            loc_lat_dim=CFG_enc['loc_lat_dim'],
            dim_space=2,
            nneigh=CFG['data']['VCA_nneigh'],
            hidden_dim=CFG_render['hidden_dim']
        )
    elif CFG_render['type'] == 'lfn':
        decoder = GlobalLFNDecoder(
            CFG_enc['lat_dim'],
            -1,  # TODO dim space
            hidden_dim=CFG_render['hidden_dim'])
    else:
...
def get_texture_renderer(CFG):
    CFG_enc = CFG['encoder']
    CFG_dec = CFG['decoder']
    CFG_render = CFG['renderer']
    if CFG_render['type'] == 'nepu':
        decoder = NePuTextureRenderer(
            lat_dim=CFG_enc['lat_dim'],
            loc_lat_dim=CFG_dec['loc_lat_dim'],
            dim_space=2,
            nneigh=CFG['data']['VCA_nneigh'],
            hidden_dim=CFG_render['hidden_dim']
        )
